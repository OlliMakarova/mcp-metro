// Orchestrator of Saint Petersburg metro data acquisition. Cascade:
//
//   1. Fresh SPb graph source (spb.metrobook.ru)  — the weighted graph core
//   2. Disk copy of the graph                     — when the source is unavailable
//   3. Nothing available → dataset = null
//
// The two enrichment sources (hh.ru reference, official metro.spb.ru hours) are best-effort
// on BOTH paths: a fresh copy is preferred, the disk copy is the fallback, and their absence
// only reduces the amount of detail (no coordinates / no working hours) without failing the
// refresh. None of the SPb files have a time-to-live: stations, ride times and working hours
// do not go stale the way Moscow closure notifications do; closure notes synthesized from the
// official page carry their own validity window instead.
//
...
/** Where the SPb data ultimately came from */
export type TSpbRefreshOrigin = 'spb-fresh' | 'spb-disk' | 'none';
...
export const loadSpbMetroDataFromDisk = async (deps: ISpbRefreshDeps): Promise<ISpbRefreshResult> => {
  ...
  const graph = await deps.storage.readMetrobookGraph('spbMetrobookGraph');
  if (!graph) {
  ...
  return { dataset: buildSpbDataset(graph, hh, official), origin: 'spb-disk' };
};
...
export const refreshSpbMetroData = async (deps: ISpbRefreshDeps): Promise<ISpbRefreshResult> => {
  ...
  if (!graph) {
    log.error('Failed to obtain SPb metro data: the graph source is unavailable and there is no disk copy');
    return { dataset: null, origin: 'none' };
  }
  ...
  const dataset = buildSpbDataset(graph, hh, official);
  log.info(
    `SPb metro data refreshed (graph ${graphFresh ? 'fresh' : 'from disk'}, hh ${hh ? 'yes' : 'no'}, official ${
      official ? 'yes' : 'no'
    }): ${dataset.stations.length} stations, ${dataset.notifications?.length ?? 0} notifications`,
  );
  return { dataset, origin: graphFresh ? 'spb-fresh' : 'spb-disk' };
};
