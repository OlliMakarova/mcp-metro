// Автономный поиск быстрейшего маршрута по данным spb.metrobook.ru (без запросов в сеть).
//
// Запуск:
...
const g = JSON.parse(await readFile(join(DIR, 'spb-metrobook-graph.json'), 'utf8'));
...
// В отличие от Москвы, в выгрузке СПб названа каждая вершина «станция × линия», поэтому
// ищем и по подписям вершин тоже (у пересадочных узлов подписи разных линий различаются:
// «Садовая», «Сенная площадь», «Спасская» — один узел, три вершины с разными именами).
function findStation(name) {
  const q = name.trim().toLowerCase().replace(/ё/g, 'е');
  const norm = (s) => (s ?? '').toLowerCase().replace(/ё/g, 'е');
  const bySdid = Object.entries(g.stationInstances);
  const exact = bySdid.filter(([, s]) => norm(s.name) === q);
  const found = exact.length ? exact : bySdid.filter(([, s]) => norm(s.name).includes(q));
  if (!found.length) {
    console.error(`Станция не найдена: «${name}»`);
    process.exit(1);
  }
  // Возвращаем все вершины физической станции первой найденной вершины
  const [sdid, inst] = found[0];
  const sids = g.stations[String(inst.stationId)];
  return { name: inst.name ?? sids?.name ?? `#${sdid}`, sdids: sids?.sdids ?? [Number(sdid)] };
}
...
function dijkstra(sources, targets) {
  ...
  const push = (id, d) => {
    heap.push({ id, d });
    heap.sort((a, b) => b.d - a.d); // для графа в ~75 вершин простая сортировка достаточна
  };
...
console.log(`\nМаршрут: ${fromSt.name} → ${toSt.name} (данные spb.metrobook.ru)`);
